class CacheStatisticsController < ApplicationController
  ...
  def total_stats
    ...
    @stats = CacheStatistic.get_n_latest_records(10)
    ...
    if params[:stats] != session[:stats] and @selected_ratings != {}
      ...
      @graph = CacheStatistic.get_selected_graph(@all_stats, 10, 10)
    else
      @selected_stat_names = @selected_stats.keys.sort
      @graph = CacheStatistic.get_selected_graph(@selected_stat_names, 10, 10)
    end
